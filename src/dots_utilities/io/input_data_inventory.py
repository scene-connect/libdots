...
class InputDataInventory(ABC):
    ...
    @abstractmethod
    def setup(self, calculation_messages: dict[str, IODataInterface]):
    ...
    def set_expected_esdl_ids_for_input_data(
        ...
        ) in connected_input_esdl_objects_dict.items():
            for service_name, esdl_ids in connected_input_esdl_objects.items():
                for esdl_id in esdl_ids:
                    if f"/data/{service_name}/model" not in self.expected_esdl_ids_dict:
                        self.expected_esdl_ids_dict[f"/data/{service_name}/model"] = [
                            esdl_id
                        ]
                    elif (
                        esdl_id
                        not in self.expected_esdl_ids_dict[
                            f"/data/{service_name}/model"
                        ]
                    ):
                        self.expected_esdl_ids_dict[
                            f"/data/{service_name}/model"
                        ].append(esdl_id)
        LOCAL_LOGGER.debug(
            f" set expecting input ESDL objects for 'home_network_with_battery': {self.expected_esdl_ids_dict}"
        )
    ...
    def add_input(
        ...
        if input_class_instance:
            if (
                ...
                and not self.expected_esdl_ids_dict
            ):
            ...
            LOCAL_LOGGER.debug(
                f" added '{data_name}' data for service 'home_network_with_battery': {input_class_instance.get_variable_descr()}"
            )
    ...
    def is_step_active(self) -> bool:
        return (
            self.input_data_dict["new_step"] != {}
            and self.input_data_dict["new_step"] != None
        )
